<?php
  class User {
    ...
    //Ambil data user dengan nomor induknya
    public static function readDosen() {
      $list = [];
      $db = Db::getInstance();
      $req = $db->query("SELECT * FROM user WHERE status = 1");

      foreach($req->fetchAll() as $post) {
        $list[] = new User($post['nomor_induk'], $post['password'], $post['nama'], $post['fakultas'], $post['jurusan'], $post['ttl'], $post['jenis_kelamin'], $post['status']);
      }
