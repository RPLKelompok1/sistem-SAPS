<?php
  class Sertifikat {
    ...
    //Ambil data sertifikat berdasarkan untuk pendaftaran SAPS
    /*1 - 63    penalaran
    64 - 103  minar dan bakat
    104 - 111 pengabdian*/
    public static function mylist_pendaftaran($nomor_induk, $status) {
      $list = [];
      $db = Db::getInstance();
      if ($status == 1) {
        $req = $db->query("SELECT * FROM sertifikat WHERE id_user = $nomor_induk AND id_category < 64");
      } elseif ($status == 2) {
        $req = $db->query("SELECT * FROM sertifikat WHERE id_user = $nomor_induk AND id_category > 63 AND id_category < 104");
      } else {
        $req = $db->query("SELECT * FROM sertifikat WHERE id_user = $nomor_induk AND id_category > 103");
      }
